...
- **PO が別の開発対象リポジトリ（pergram）でトライアル運用中。** 2026-09-02 に導入した（それ以前の対象は SNS Agent）。**pergram は既に動いている案件であり、導入前からタスク台帳を持っていた**——別方式の台帳を組織の形式へ移す手順を実際に通した最初のケースである。導入で配布物側の課題が3件出ている（引き継ぎ記録の「トライアル対象を pergram に決め、導入した」）。**組織が実際に動いて初めて分かることが、ここから入ってくる。**
...
1. ~~**トライアル先へ、修正した配布物を反映する**~~ **→ pergram については完了（2026-09-02）。現時点の配布物一式をそのまま入れた。** 代わりに、**導入で見つかった配布物の課題3件を直す**——(a) 導入手順書の確認欄が古い（担当エージェントは5本ではなく6本、スキルは4本ではなく5本）、(b) 整合性検査の警告文言「指示が未記入のまま担当が付いている」が実際の判定条件と食い違う（担当が未割当でも出る。`org/scripts/org-check.py:378`）、(c) 導入手順に `.claude/scripts/__pycache__/` を `.gitignore` へ足す指示が無い。**以下は旧トライアル先（SNS Agent）を今後も動かす場合にだけ残る作業である** — 2026-08-25 の修正6件と、**トークン集計の一式（`org/scripts/org-tokens.py`・`org/settings.snippet.json`・`org/CLAUDE.md`・`org/README.md`・`org/skills/org-orchestrate/`）**と、**稼働状況のモニタ（`org/scripts/org-monitor.py`）**が届いていない。手でコピーし直す。**停滞検知スクリプトは、トライアル先が同じ不具合を独自に直しているため突き合わせが要る**
