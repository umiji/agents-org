...
- **決定ではなく、決定の根拠。** Claude Code の機能制約と、複数エージェント構成に関する調査の記録
- 特に効いている事実: 子プロセスとして起動されたエージェントは**人間へ質問できず、親の会話も他エージェントの会話も見えない**／`CLAUDE.md` は**強制力を持たず、200行を超えると遵守率が落ちる**
- 失敗モードの一覧（指示不足が沈黙して伝播する、検証が最初の成功で止まる、司令塔が委譲せず自分で作業する）は、01〜13 の各所で対策の根拠になっている
